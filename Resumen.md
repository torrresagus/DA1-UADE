...
- **Estado actual (2026-07-01):** todas las funcionalidades implementadas y probadas, pendiente deploy del backend
...
- `POST /{id}/resolver` → empresa acepta/rechaza + propone precio/comisión
...
| `procesar_solicitudes_pendientes()` | INGRESADA +15s → EN_INSPECCION; EN_INSPECCION +30s desde creación → ACEPTADA |
...
| Solicitud: INGRESADA → EN_INSPECCION | **15 segundos** | `_TIMER_SOLICITUD` |
| Solicitud: EN_INSPECCION → ACEPTADA | **30 segundos** desde creación | `_TIMER_SOLICITUD * 2` |
...
- `toLotCard()` incluye `moneda: subasta.moneda` — usado por `AuctionCard` para mostrar `$precio ARS/USD`
...
9. **Subir bien:** upload-product (con selector moneda y checkbox multi-elemento) → solicitud → scheduler avanza estados (~15s y ~30s) → product-status → aceptar propuesta (con checkbox "iniciar ya")
...
| **Solicitud flow** | INGRESADA → EN_INSPECCION (+15s) → ACEPTADA (+30s desde creación) |
...
## 13. Gotchas y bugs conocidos (LEER ANTES DE TOCAR)
...
## 14. Seed de datos de prueba (`seed.py`)
